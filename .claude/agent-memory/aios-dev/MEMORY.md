...
- `UnifiedActivationPipeline` in `.aios-core/development/scripts/unified-activation-pipeline.js` is the single entry point for ALL 12 agents
...
- `aios-master` uses string format `visibility: full` instead of array
...
- `PermissionMode` and `OperationGuard` in `.aios-core/core/permissions/` are fully functional

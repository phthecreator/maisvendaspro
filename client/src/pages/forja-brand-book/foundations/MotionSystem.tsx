...
const easings = [
  ...
  { name: 'forge', value: 'steps(4, end)', desc: 'Stepped/impact' },
];
...
const MotionSystem = () => {
  ...
  const [playingHeatReveal, setPlayingHeatReveal] = useState(false);
  const [playingForgeStrike, setPlayingForgeStrike] = useState(false);
  ...
  return (
    <section>
      <style>{`
        @keyframes forja-fade-in { from { opacity: 0; } to { opacity: 1; } }
        @keyframes forja-slide-up { from { opacity: 0; transform: translateY(24px); } to { opacity: 1; transform: translateY(0); } }
        @keyframes forja-scale-in { from { opacity: 0; transform: scale(0.85); } to { opacity: 1; transform: scale(1); } }
        @keyframes forja-forge-glow {
          0% { opacity: 0; filter: brightness(0.3); }
          20% { opacity: 0.4; filter: brightness(2.5); }
          40% { opacity: 0.7; filter: brightness(1.8); }
          60% { opacity: 0.85; filter: brightness(1.3); }
          80% { opacity: 0.95; filter: brightness(1.1); }
          100% { opacity: 1; filter: brightness(1); }
        }
        @keyframes forja-heat-reveal {
          from { clip-path: inset(0 100% 0 0); filter: brightness(1); }
          50% { filter: brightness(1.8); }
          to { clip-path: inset(0 0 0 0); filter: brightness(1); }
        }
        @keyframes forja-forge-strike {
          0% { transform: scale(1); filter: brightness(1); }
          10% { transform: scale(1.15); filter: brightness(3); }
          20% { transform: scale(0.95); filter: brightness(0.8); }
          40% { transform: scale(1.05); filter: brightness(1.5); }
          60% { transform: scale(0.98); filter: brightness(1.1); }
          100% { transform: scale(1); filter: brightness(1); }
        }
        .forja-motion-box { transition: transform 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
      `}</style>
      ...
      <h2 className="text-[42px] font-bold leading-tight mb-6"
        style={{ fontFamily: "'Averia Serif Libre', serif", color: '#FFF8F0' }}>Tokens de Animacao</h2>
      ...
      <h3 className="text-2xl font-bold mb-6"
        style={{ fontFamily: "'Averia Serif Libre', serif", color: '#FFF8F0' }}>Escala de Duracao</h3>
      <p className="text-[13px] text-[#B8976A] mb-4" style={{ fontFamily: "'Roboto Mono', monospace" }}>
            ...
            <div className="flex-1 h-8 rounded relative overflow-hidden"
              ...
              <div className="h-full rounded transition-all"
                style={{
                  ...
                  background: 'linear-gradient(90deg, rgba(255,107,0,0.4), rgba(255,107,0,0.8))',
      ...
      {/* Easing Curves */}
      <h3 className="text-2xl font-bold mb-6"
        style={{ fontFamily: "'Averia Serif Libre', serif", color: '#FFF8F0' }}>Curvas de Easing</h3>
      <p className="text-[13px] text-[#B8976A] mb-4" style={{ fontFamily: "'Roboto Mono', monospace" }}>
          ...
          <div className="h-32 flex items-center justify-center">
            <div className="text-2xl font-bold"
              style={{
                ...
                color: '#FF6B00',
                animation: playingHeatReveal ? 'forja-heat-reveal 1s ease-out forwards' : 'none',
                clipPath: playingHeatReveal ? undefined : 'inset(0 0 0 0)',
              }}>
              HEAT REVEAL
            </div>
          </div>
          ...
          <div className="h-32 flex items-center justify-center">
            <div className="text-2xl font-bold"
              style={{
                fontFamily: "'Averia Serif Libre', serif",
                color: '#FFD700',
                animation: playingForgeStrike ? 'forja-forge-strike 0.6s ease-out forwards' : 'none',
              }}>
              FORGE STRIKE
            </div>
          </div>
      ...
      {/* Reduced Motion Note */}
      <div className="p-5 rounded-lg border border-white/5" style={{ background: 'rgba(255,168,0,0.06)', borderColor: 'rgba(255,168,0,0.15)' }}>
        <div className="text-[11px] text-[#FFA800] uppercase tracking-[0.1em] mb-2"
          style={{ fontFamily: "'Roboto Mono', monospace" }}>Acessibilidade</div>
        <div className="text-[13px] text-[#FFF8F0]" style={{ fontFamily: "'Roboto Mono', monospace" }}>
          Todas as animacoes devem respeitar <span className="text-[#FF6B00]">@media (prefers-reduced-motion: reduce)</span>. Quando ativo, substituir animacoes por transicoes instantaneas ou remove-las completamente. Manter opacity transitions como fallback.
        </div>
      </div>
    </section>
  );
